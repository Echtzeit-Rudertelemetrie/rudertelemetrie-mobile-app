...
void main() {
  test('emits once per aligned timestamp pair, using both latest values', () async {
  ...
  test('does not emit while a timestamp is unaligned beyond tolerance', () async {
    final a = PushDataSource(name: 'A', unit: Unit.N);
    final b = PushDataSource(name: 'B', unit: Unit.deg);
    final combined = CombineLatestSource(
      name: 'A+B',
    ...
    a.add(Measurement(value: 3, timestamp: DateTime.fromMillisecondsSinceEpoch(1000)));
    // 100 ms apart, far beyond the 5 ms alignment tolerance.
    b.add(Measurement(value: 4, timestamp: DateTime.fromMillisecondsSinceEpoch(1100)));
    await pumpEventQueue();
